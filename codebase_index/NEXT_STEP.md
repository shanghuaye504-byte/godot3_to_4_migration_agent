...
.tscn/.tres
    │
    ▼
L0 lexer.py ──► L1 model.py ──► L2 resolve.py ──► L3 store.py
...
| L3  | `scene/store.py`    | `SceneModel → SQLite`                                  | 有 IO。挂在现有 writer 事务里  |
...
新代码都在 `codebase_index/index/src/codeindex/scene/`，测试在 `codebase_index/index/tests/scene/`。不新开 `pyproject.toml`，不改 `godot_mcp`，不改 GDScript parser。
...
已有文件里的 `SCENE_INDEX_TODO(<id>)` 表示「以后在这一处改」。标记本身不改变行为，也不是实现。

格式：

```text
SCENE_INDEX_TODO(<id>): 见 codebase_index/NEXT_STEP.md 第 7 节开头的「标记」说明。
```

某一整步的验收都通过之后：删掉该步对应的标记，再把该步的勾选框改成 `[x]`。不要在只放下骨架、还没写函数的时候勾选 Step 1–5。

| id | 文件 | 以后要加的行为 |
| --- | --- | --- |
| `step3-schema` | `index/src/codeindex/db/schema.sql` 文末 | 不改 `files` / `symbols` / `edges` / `classes`。场景建表在 `scene/schema.sql`，由写连接额外执行 |
| `step3-connection` | `index/src/codeindex/db/connection.py` 的 `connect_rw` | 现有 `schema.sql` 之外再执行 `scene/schema.sql`。只读连接仍然不建表 |
| `step3-writer` | `index/src/codeindex/db/writer.py` 的 `reindex_batch` | 按后缀分叉。`.tscn` / `.tres` 走 `scene/store.py`，不进 tree-sitter，不进 `files` |
| `step3-scan` | `index/src/codeindex/daemon.py` 的 `_should_index` | 全量扫描和 watcher 都看见这两种后缀。`_watch_filter` 已复用该函数，不必改 `watcher.py` |
| `step3-find-symbol` | `index/src/codeindex/db/queries.py` 的 `find_symbol` | 追加场景引用。`matches` 的列保持不动 |
| `step3-find-symbol-exit` | `index/src/codeindex/cli.py` 的 `cmd_find_symbol` | 没有代码符号但有场景引用时退出码 0 |
| `step4-rules-db` | `index/src/codeindex/config.py` 与 `config.example.yaml` | 可选字段 `migration_rules_db`。缺省时不报 `E_UNMIGRATED_TYPE` |
| `step5-scene-check` | `index/src/codeindex/cli.py` 的 `build_parser` | 增加 `scene-check`。跑完是退出码 0，库没准备好是 2。不用 1 表示有 regression |

### 清单

- [x] 骨架：下面列出的新文件已经放好，里面只有注释；上表的标记已经加在已有文件里。还没有函数实现，也没有跑测试。

- [ ] Step 1 — L0 + L1。生产路径靠 span 切原文，不靠把文件打印回去。

  新建（目前只有注释）：

  - `index/src/codeindex/scene/__init__.py`：包标记。实现后从此导出 L0/L1 的公开类型。
  - `index/src/codeindex/scene/README.md`：本包职责与「尚未实现」。
  - `index/src/codeindex/scene/lexer.py`：把文本切成带 span 的值。不判断类型是否存在，不读磁盘。
  - `index/src/codeindex/scene/model.py`：把值收成 `Section` / `SceneDoc`，并记下语法级 `P_*`。
  - `index/tests/scene/README.md`：测试目录说明。
  - `index/tests/scene/test_lexer_model.py`：本步验收的空测试。

  不改已有代码。

  夹具：八个文件都跑 span 与不抛。格式矩阵的特殊写法以两个 `generated_format_matrix.tscn` 为准。六个 `real_dodge_*.tscn` 必须零语法错误。预期与通过定义见 `codebase_index/scenecheckfixture/README.md`。

  验收（实现后才勾选）：

  - [ ] 每个 `Value` / `Section` 的 `text[byte_start:byte_end] == raw`；`line` / `col` 与逐字节换行计数一致。
  - [ ] 去掉注释和空白后，每个字节都落在某个 span 里。
  - [ ] 格式矩阵全过：format 2 与 3、整数 id 与字符串 id、`;` 注释、多行 Packed 数组、`theme_override_colors/font_color`、`libraries/""`、`&"..."`、`^"..."`、`Array[Node]([])`、转义、空 `NodePath("")`、尾随逗号、CRLF、无尾换行。
  - [ ] 语料加随机截断 / 字节翻转：零未捕获异常，问题进 `problems[]`。
  - [ ] 每个 `P_*` 至少 1 个正例 + 1 个易混淆反例。
  - [ ] `PoolByteArray(...)` / `NotARealType(...)` 保留为 call，不报语法错误。

- [ ] Step 2 — L2。单文件关系，不碰文件系统。

  新建（目前只有注释）：

  - `index/src/codeindex/scene/resolve.py`：节点全路径、实例子树、引用边。不 import `os` / `pathlib` / `sqlite3`。
  - `index/tests/scene/test_resolve.py`：本步验收的空测试。

  不改已有代码。

  夹具：八个文件的根数量都是 1。继承和 `instance_placeholder` 只断言两个 `generated_format_matrix.tscn`。`ext_use` / `sub_use` 计数对全部八个文件做。两份 `real_dodge_main` 的 `MobSpawnLocation` 父路径是 `MobPath`。详见 `codebase_index/scenecheckfixture/README.md`。

  验收：

  - [ ] 含多层嵌套、`parent="."`、同名兄弟的 fixture，`node_by_path` 与人工标注一致。
  - [ ] 0、1、2 个根时 `root_paths` 长度正确。
  - [ ] 继承场景、`instance=`、`instance_placeholder` 的 `unresolved_subtrees` / `is_inherited` 与标注一致。
  - [ ] 语料上 `ext_use` / `sub_use` 的条数，等于在字符串之外数到的 `ExtResource("` / `SubResource("` 次数。
  - [ ] `resolve.py` 的导入不含 `os` / `pathlib` / `sqlite3`。

- [ ] Step 3 — L3 + `find-symbol`。

  新建（目前只有注释）：

  - `index/src/codeindex/scene/schema.sql`：`scene_files` / `scene_nodes` / `scene_ext` / `scene_sub` / `scene_conn` / `scene_refs` / `scene_baseline` / `scene_member_baseline`。实现时才写成可执行 SQL，现在是注释。
  - `index/src/codeindex/scene/store.py`：在调用方已经打开的事务里写入和删除场景行。超限与二进制只记 `parse_status`，不喷语法错误。
  - `index/tests/scene/test_store.py`：路径、同事务、增量等于全量、超限与二进制。
  - `index/tests/scene/test_find_symbol_scene.py`：场景引用、空 matches 的退出码、无场景时的向后兼容。

  要改的已有文件（现在只有标记）：`step3-schema`、`step3-connection`、`step3-writer`、`step3-scan`、`step3-find-symbol`、`step3-find-symbol-exit`。

  夹具：相对路径用两个 generated 里的 `icon.png`，应落到该 `.tscn` 所在目录。真实文件里的 `res://` 在夹具目录下没有对应文件时，`resolved_file` 为空是对的，不因此判失败。详见 `codebase_index/scenecheckfixture/README.md`。

  验收：

  - [ ] `res://`、相对路径、`::` 后缀的 `resolved_file` 正确。只有 uid、没有 path 时为 NULL，且不产生 `E_EXT_PATH_MISSING`。
  - [ ] 一批里夹一个坏场景，其他 `.gd` 与好场景仍然提交；坏场景是 `MALFORMED`，不是整批回滚。
  - [ ] 随机编辑后，增量 sync 的 `scene_*` 与全量重建逐行一致。
  - [ ] daemon 全量扫描与 watcher 能看见 `.tscn` / `.tres`；这些文件不出现在 `files` 表。
  - [ ] 单文件 > 4MB 为 `SKIPPED_OVERSIZE`；含 NUL 为 `BINARY`。二者进入 coverage，不产生 P 级洪水。
  - [ ] 一个 `class_name`、一个 `func`、一个脚本路径在场景里被引用时，`find-symbol` 都能带回。`scene_usage_count` 等于全表计数；大于 50 时 `scene_usage_truncated=true`。
  - [ ] 符号已从 `.gd` 删除、场景仍引用：退出码 0，`matches=[]`，`scene_usages` 非空。
  - [ ] 无场景引用时，`matches` 的字段与改之前一致（`line` 仍是 0-based）。不要求整个 JSON 逐字节相同。
  - [ ] 不破坏旧实现：重跑 `index/tests/test_queries.py`、`index/tests/test_connection.py`、`index/tests/test_watcher_debounce.py`，全部仍通过。`call-chain` 与 `class-hierarchy` 的返回不变。

- [ ] Step 4 — L4 ERROR。每次现算，不缓存 problem。

  新建（目前只有注释）：

  - `index/src/codeindex/scene/rules.py`：文件内闭合、路径、旧类名黑名单；按 `node_path` 弃权；规则库缺失时跳过 `E_UNMIGRATED_TYPE`。
  - `index/tests/scene/test_rules.py`：本步验收的空测试。

  要改的已有文件（现在只有标记）：`step4-rules-db`。`godot_mcp` 读同一份 yaml 时忽略这个键，不必改它。

  夹具：四个 Godot 3 文件都有 `E_NOT_CONVERTED`，这不是解析失败。四个 Godot 4 文件 `format` 为 3，其中三个真实文件没有语法级 ERROR。`KinematicBody2D` 是否报 `E_UNMIGRATED_TYPE` 取决于是否配置了规则库。弃权用 generated 的继承根和 `instance_placeholder`。能 load 就不报 ERROR 的那一项不要用这 8 个孤立文件。详见 `codebase_index/scenecheckfixture/README.md`。

  验收：

  - [ ] 小语料里 Godot 能 `ResourceLoader.load` 成功的文本场景，不报任何 ERROR。只 load，不 instantiate。单文件超时；进程崩溃则记下该文件并从下一个续跑。不要求「load 失败就必须报 ERROR」。
  - [ ] 在干净文件上注入每一类 E 级故障，每类多例。检出率 100%，`code` 正确，`subject` 不含行号。
  - [ ] 同样的注入放进实例子树或继承场景：ERROR 数为 0，且 `suppressed_checks` 用 `node_path` 记下来。
  - [ ] 仓库里有 `class_name KinematicBody2D` 时，不报 `E_UNMIGRATED_TYPE`。
  - [ ] 不配置 `migration_rules_db` 时不报 `E_UNMIGRATED_TYPE`，`rules_unavailable=true`。
  - [ ] 文件任意位置插删空行或注释，已有 problem 的 fingerprint 不变。
  - [ ] 同输入跑多次，problem 集合一致。
  - [ ] 不破坏旧实现：重跑 `index/tests/test_config.py`。没有 `migration_rules_db` 的旧 yaml 仍能加载。

- [ ] Step 5 — L5 + `scene-check`。

  新建（目前只有注释）：

  - `index/src/codeindex/scene/baseline.py`：冻结 fingerprint 与 class/func 成员快照；三分法；`E_MEMBER_GONE`。快照冻结后不随重新索引更新。
  - `index/src/codeindex/scene/check.py`：组装 `scene-check` 的 JSON。`--path` 的闭包用 `scene_refs.target_file` 反查。
  - `index/tests/scene/test_baseline.py`：三分法、行号漂移、成员消失、引擎类型、脚本损坏弃权、快照不跟随索引。
  - `index/tests/scene/test_scene_check_cli.py`：路径闭包与退出码。

  要改的已有文件（现在只有标记）：`step5-scene-check`。

  夹具：若把当前 8 个文件冻成 baseline，Godot 3 的 `E_NOT_CONVERTED` 算旧账，不是 regression。成员消失仍用另建的 `.gd`，不要改这 8 个文件来做那项测试。详见 `codebase_index/scenecheckfixture/README.md`。

  验收：

  - [ ] 冻结后修好若干、弄坏若干、留下若干：`fixed` / `regression` / `pre_existing` 的个数精确。
  - [ ] 文件头插入 100 行后，旧问题仍是 pre_existing，`regression` 为空。
  - [ ] 冻结后把 `class_name Player` 改名，场景 `type="Player"` 仍在 → 一条 regression。场景改成新名字后这条消失。
  - [ ] 冻结后重命名被 `[connection method=]` 指向的函数，且没改场景 → regression。引擎回调 `_ready` 不报。
  - [ ] 场景里的 `Sprite2D` / `CharacterBody2D` 不产生 `E_MEMBER_GONE`。
  - [ ] 目标 `.gd` 语法失败时，不报 `E_MEMBER_GONE`，有 `suppressed_checks`。
  - [ ] 传入 `a.gd` 时，结果包含引用它的场景上的问题，不漏；不包含无关场景上新造的问题。
  - [ ] 有 regression 时退出码仍是 0；db 不存在时是 2。
  - [ ] baseline 冻结后再索引，`scene_member_baseline` 仍是冻结时的成员，不是最新符号表。
  - [ ] 不破坏旧实现：重跑 `index/tests/test_queries.py`。无场景引用时 `find-symbol` 的 `matches` 与退出码与改之前一致；`call-chain` 与 `class-hierarchy` 不变。
